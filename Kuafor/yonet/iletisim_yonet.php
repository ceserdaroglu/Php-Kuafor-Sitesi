...
<?
	...
	if($_GET['islem']=='edit')
	{
		$sorgu_edit=$db->prepare("Update tbl_iletisim set Icerik=? where IletisimId=?");
		$edit=$sorgu_edit->execute(array($_POST['icerik'],$_GET['id']));
	}
?>
...
<body>
	...
	<div class="row">
    	<div class="col-md-12 text-center">
            ...
            <a class="btn btn-link" href="hakkimizda.php">Resimler</a>
        ...
        <div class="row">
        	<div class="col-md-12">
                ...
                <div class="list-group">
                	<?
						$query=$db->query("Select * from tbl_iletisim",PDO::FETCH_ASSOC);
						if($query->rowCount())
						{
							foreach($query as $row)
							{
								print "<a href='?id=$row[IletisimId]'><li class='list-group-item'>".$row['Icerik']."</li></a>";
							}
						}
					?>
                </div>
        ...
        <?
			if(isset($_GET['id']))
			{
				$sorgu=$db->prepare("Select * from tbl_iletisim where IletisimId=?");
